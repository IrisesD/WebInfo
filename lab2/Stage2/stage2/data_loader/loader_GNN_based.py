...
class DataLoader(DataLoaderBase):
    ...
    def construct_data(self, kg_data):
        ...
        kg_data = pd.concat(frame, ignore_index=True)
        ...
        self.cf_train_data = (np.array(list(map(lambda x : x + self.n_entities, self.cf_train_data[0]))), self.cf_train_data[1])
        self.cf_test_data = (np.array(list(map(lambda x : x + self.n_entities, self.cf_test_data[0]))), self.cf_test_data[1])

        # 4. 将 self.train_user_dict 和 self.test_user_dict 中的用户索引（即key值）范围从[0, num of users)
        #    映射到[num of entities, num of entities + num of users)，并保持原有数据形式和结构不变
        train_dic = {}
        for k, v in self.train_user_dict.items():
            train_dic[k+self.n_entities] = v
        self.train_user_dict = train_dic
        test_dic = {}
        for k, v in self.test_user_dict.items():
            test_dic[k+self.n_entities] = v
        self.test_user_dict = test_dic
        ...
        for index, row in self.kg_train_data.iterrows():
            self.train_kg_dict[row[0]].append((row[2], row[1]))
            self.train_relation_dict[row[1]].append((row[0], row[2]))
    ...
    def create_laplacian_dict(self):
        ...
        def random_walk_norm_lap(adj):
            ...
            norm_adj = d_mat_inv_sqrt.dot(adj)
            return norm_adj.tocoo()
