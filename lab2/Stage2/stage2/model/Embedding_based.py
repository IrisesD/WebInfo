...
class Embedding_based(nn.Module):
    ...
    def calc_kg_loss_TransR(self, h, r, pos_t, neg_t):
        ...
        pos_score = torch.power(torch.norm(r_mul_h + r_embed - r_mul_pos_t, p=2, dim=1), 2)                                                         # (kg_batch_size)
        neg_score = torch.power(torch.norm(r_mul_h + r_embed - r_mul_neg_t, p=2, dim=1), 2)                                                           # (kg_batch_size)
    ...
    def calc_cf_loss(self, user_ids, item_pos_ids, item_neg_ids):
        ...
        item_pos_cf_embed = item_pos_embed * item_pos_embed
