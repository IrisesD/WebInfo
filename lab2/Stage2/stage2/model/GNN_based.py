...
class GNN_based(nn.Module):
    ...
    def calc_cf_embeddings(self):
        ...
        for idx, layer in enumerate(self.aggregator_layers):
            ...
            temp = F.normalize(ego_embed, p=2, dim=1)
            all_embed.append(temp)                                                                 # (n_users + n_entities, embed_dim)
    ...
    def calc_kg_loss_TransR(self, h, r, pos_t, neg_t):
        ...
        r_mul_neg_t = torch.bmm(neg_t_embed.unsqueeze(1), W_r).squeeze(1)  # (kg_batch_size, relation_dim)                                                                  # (kg_batch_size, relation_dim)

        # 8. 对关系嵌入，头实体嵌入，尾实体嵌入，负采样的尾实体嵌入进行L2范数归一化
        r_embed = r_embed / torch.norm(r_embed, p=2, dim=1, keepdim=True) 
        r_mul_h = r_mul_h / torch.norm(r_mul_h, p=2, dim=1, keepdim=True)
        r_mul_pos_t = r_mul_pos_t / torch.norm(r_mul_pos_t, p=2, dim=1, keepdim=True) 
        r_mul_neg_t = r_mul_neg_t / torch.norm(r_mul_neg_t, p=2, dim=1, keepdim=True) 

        # 9. 分别计算正样本三元组 (h_embed, r_embed, pos_t_embed) 和负样本三元组 (h_embed, r_embed, neg_t_embed) 的得分
        pos_score = torch.sum(torch.pow(r_mul_h + r_embed - r_mul_pos_t, 2), dim=1)  # (kg_batch_size)
        neg_score = torch.sum(torch.pow(r_mul_h + r_embed - r_mul_neg_t, 2), dim=1)  # (kg_batch_size)                                                                      # (kg_batch_size)

        # 10. 使用 BPR Loss 进行优化，尽可能使负样本的得分大于正样本的得分
        kg_loss = torch.mean(-F.logsigmoid(pos_score - neg_score))
    ...
    def calc_kg_loss_TransE(self, h, r, pos_t, neg_t):
        ...
        pos_score = torch.norm(h_embed + r_embed - pos_t_embed, p=1, dim=-1)                                                                          # (kg_batch_size)
        neg_score = torch.norm(h_embed + r_embed - neg_t_embed, p=1, dim=-1)                                               # (kg_batch_size)
